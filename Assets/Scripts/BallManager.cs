...
public class BallManager : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.tag == "Racket")
        {
            ...
            Vector2 Dir = new Vector2((transform.position.x - collision.gameObject.transform.position.x), (transform.position.y - collision.gameObject.transform.position.y)*angleMultp);
            Dir.Normalize();
